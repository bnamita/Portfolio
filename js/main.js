...
function render(viz_list) {
    ...
    var viz = d3.select(".projects").selectAll(".viz")
        ...
        //    return d.title;
        //});
    d3.select(".projects").selectAll(".viz")
        .append("div")
        .attr("class", "caption__overlay")
        .append("p")
        .attr("class", "caption__overlay__content")
        .html(function(d) {
            return d.desc + ' <br><br><a style="text-decoration: underline;" target="_blank" href="//bnamita.github.io/' + d.name + '"> View Interactive Visualization</a>';
        });
...
function render_single(viz_list, elem) {
    ...
    var viz = d3.select("." + elem).selectAll(".viz")
        .data(viz_list, function(d) { return d.id; })
        .enter().insert("a", "br")
